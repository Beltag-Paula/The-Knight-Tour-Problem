...
solution = [[0 for _ in range(chessboard_size)] for _ in range(chessboard_size)]  # Define solution globally
    ...
    for row in range(chessboard_size):
        for col in range(chessboard_size):
            ...
            if step > 0:  # Check if step is greater than 0 to display the number
                canvas.create_text(col * 50 + 25, row * 50 + 25, text=str(step), font=("Arial", 16))
...
def next_step():
    ...
    try:
        ...
        draw_board(solution)
        step += 1
        root.after(200, next_step)  # Automate next step after 200 milliseconds (adjust as needed)
